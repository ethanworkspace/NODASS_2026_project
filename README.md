...
本專案用於監測台灣海岸是否發生水質污染事件，並在污染發生後，依事件日前後 72 小時內的海氣象條件預測污染可能擴散到哪裡。系統以透明可檢核的規則式模型作為初版核心，並保留後續升級為貝氏網路、因果推論、時序分類與粒子漂流模型的工程架構。
...
- 事件表會列出測站、時間、污染物、實測值、基準值與異常分數。

### 2. 72 小時時空一致性

- 每個污染事件自動建立事件日前後 72 小時事件窗。
- 只使用事件窗內的海氣象、水文與影像查詢條件。
- 擴散預測使用順流、下風與可達距離做判斷。
- 事件窗外資料不納入模型判斷，避免用不同水文條件混合解釋同一污染事件。
...
- 多測站時空異常偵測：對應污染事件偵測與事件窗。
- 水動力污染傳輸概念：對應 72 小時順流擴散軌跡。
- Bayesian/MCMC 污染推估概念：目前以透明分數與風險節點展示，後續可升級為後驗機率。
- 主動採樣策略：對應污染測站複測與擴散方向斷面採樣。
...
| `data/processed/anomaly_events.csv` | 污染異常事件 |
...
| `MOENV_API_KEY` | 環境部水污染源許可及申報資料 | https://data.moenv.gov.tw/api/v2/EMS_S_03 |
